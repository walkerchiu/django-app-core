...
from django_app_core.decorators import google_captcha3, within_validity_period
from django_app_core.graphql_jwt.refresh_token.shortcuts import (
...
if DJANGO_APP_ACCOUNT_INSTALLED:
    from django_app_account.backends import AuthBackend
...
def token_auth(f):
    ...
    def wrapper(cls, root, info, password, **kwargs):
        ...
        if not DJANGO_APP_ACCOUNT_INSTALLED:
            raise Exception("django-app-account is not installed!")
        elif endpoint not in ("hq", "dashboard", "website"):
